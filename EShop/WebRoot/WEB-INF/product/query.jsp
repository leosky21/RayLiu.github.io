		...
		$('#dg').datagrid({
			...
			toolbar : [ {
				...
				handler : function() {
					parent.$("#win").window({
						...
						width : 650,
						height : 600,
						content : '<iframe src="' + shop + '/send/main/product/save" frameborder="0" width="100%" height="100%"/>'
					});
				}
			}, '-', {
				iconCls : 'icon-edit',
				...
				handler : function() {
					...
					if (rows.length == 0) {
						...
						parent.$("#win").window({
							...
							width : 650,
							height : 600,
							content : '<iframe src="' + shop + '/send/main/product/update" frameborder="0" width="100%" height="100%"/>'
						});
					...
					if (rows.length == 0) {
						...
						$.messager.confirm('删除的确认对话框', '您确定要删除此项吗？', function(r) {
							if (r) {
								...
								//2. 发送ajax请求
								$.post(shop + "/product/deleteByIds.action", {
									ids : ids
								}, function(result) {
									if (result == "true") {
			...
			columns : [ [
				...
				},
				{
					...
					width : 100,
					height : "10",
					formatter : function(value, row, index) { //data为返回的数据，在这里做数据绑定
						/* $("#images").attr("src", "data:image/gif;base64," + data); */
						if (row.pic != null) {
							/* http://blog.csdn.net/doulinxu/article/details/59636710 */
							/* 	return "<img src='getImage?id=xxx' value='"+value+"'" ; */
