...
<body>
	<!-- multipart/form-data  modelAttribute="product" -->
	<form title="更新商品" id="ff" method="post" enctype="multipart/form-data">
		...
		<div>
			<label for="pic">更新图片:</label> <input type="file" name="pic" />
			<!-- 2. fileImage.upload -->
		</div>
		<div>
		...
		<div>
			<label for="commend">推荐商品:</label> 是:<input type="radio"
				name="commend" value="true" />&nbsp; 否:<input type="radio"
...
<script type="text/javascript">
	...
	$(function() {
		...
		$("#cc").combobox({
			...
			url : shop + '/category/productQuery.action',
			//id
			valueField : '0',
			//type
			textField : '1', //我们下拉列表中显示的是商品的类别名
		...
		$("#btn").click(function() {
			...
			if ($("#ff").form("validate")) {
				//调用submit方法提交数据
				$("#ff").form('submit', {
					url : shop + '/product/update.action', //提交时将请求传给productAction的update方法执行
					success : function() {
						//如果成功了，关闭当前窗口，并刷新页面
