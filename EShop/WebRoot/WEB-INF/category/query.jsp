...
<title>e+商城后台</title>
<link rel="stylesheet"
	href="${pageContext.request.contextPath  }/jquery-easyui-1.3.5/themes/icon.css" type="text/css"></link>
<link rel="stylesheet"
...
<script type="text/javascript">
	...
	$(function() {
		$('#dg').datagrid({
			...
			url : shop + '/category/queryJoinAccount.action',
			loadMsg : '莫急莫急,正在加油......',
			method : "post",
			queryParams : {
				type : ''
			...
			toolbar : [ {
				...
				handler : function() {
					parent.$("#win").window({
						...
						width : 350,
						height : 200,
						content : '<iframe src="' + shop + '/send/main/category/save" frameborder="0" width="100%" height="100%"/>'
					});
				}
			}, '-', {
				iconCls : 'icon-edit',
				...
				handler : function() {
					...
					if (rows.length == 0) {
						...
						parent.$("#win").window({
							...
							width : 350,
							height : 250,
							content : '<iframe src="' + shop + '/send/main/category/update" frameborder="0" width="100%" height="100%"/>'
						});
					//2. 
					}
				}
					...
					if (rows.length == 0) {
						...
						$.messager.confirm('删除的确认对话框', '您确定要删除此项吗？', function(r) {
							if (r) {
								...
								//2. 发送ajax请求
								$.post(shop + "/category/deleteByIds.action", {
									ids : ids
								}, function(result) {
									if (result == "true") {
