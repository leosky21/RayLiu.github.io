...
<head>
    ...
    <script src="${pageContext.request.contextPath}/js/getReaderBackInfo.js"></script>
    ...
    <div class="row">
        ...
        <div class="col-md-2 bootstrap-admin-col-left">
            <ul class="nav navbar-collapse collapse bootstrap-admin-navbar-side">
                <li>
                    <a href="${pageContext.request.contextPath}/bookAction_findBookByPage.action"><i
                            class="glyphicon glyphicon-chevron-right"></i> 图书查询</a>
                </li>
                <c:if test="${sessionScope.reader!=null}"><!-- 判断是否登录 -->
                <li class="active">
                    <a href="${pageContext.request.contextPath}/reader/borrowAction_findMyBorrowInfoByPage.action"><i
                            class="glyphicon glyphicon-chevron-right"></i> 借阅信息</a>
                </li>
                </c:if>
                <c:if test="${sessionScope.reader!=null}"><!-- 判断是否登录 -->
                <li>
                    <a href="${pageContext.request.contextPath}/reader/forfeitAction_findMyForfeitInfoByPage.action"><i
                    ...
                    <div class="panel panel-default bootstrap-admin-no-table-panel">
                        ...
                        <div class="bootstrap-admin-no-table-panel-content bootstrap-admin-panel-content collapse in">
                            <form class="form-horizontal"
                                  action="${pageContext.request.contextPath}/reader/borrowAction_queryBorrowSearchInfo.action"
                                  method="post">
                                <div class="row">
                                    <div class="col-lg-5 form-group">
                                        ...
                                        <div class="col-lg-8">
                                            <input class="form-control" name="borrowId" type="text" value=""
                                                   placeholder="请输入借阅编号">
            ...
            <div class="row">
                <div class="col-lg-12">
                    <table id="data_list" class="table table-hover table-bordered" cellspacing="0" width="100%">
                        ...
                        <!---在此插入信息-->
                        <c:if test="${requestScope.pb.beanList!=null}">
                            <c:forEach items="${requestScope.pb.beanList}" var="back">
                                <tbody>
                                <td><c:out value="${back.borrowId}"/></td>
                                <td><c:out value="${back.borrowInfo.book.ISBN}"/></td>
                                <td><c:out value="${back.borrowInfo.book.bookName}"/></td>
                                <td><c:out value="${back.borrowInfo.reader.paperNO}"/></td>
                                <td><c:out value="${back.borrowInfo.reader.name}"/></td>
                                <td><c:out value="${back.backDate}"/></td>
                                <td><c:out value="${back.borrowInfo.endDate}"/></td>
                                <td>
                                    <button type="button" class="btn btn-info btn-xs" data-toggle="modal"
                                            data-target="#findBackModal"
                                            onclick="getBackInfoById(<c:out value="${back.borrowId}"/>)">查看
                                    </button>
                                </td>
                                </tbody>
                            </c:forEach>
                        </c:if>
                        <c:if test="${request.pb.beanList==null}">
                            <tbody>
                            <td>暂无数据</td>
                            <td>暂无数据</td>
                            <td>暂无数据</td>
                    ...
                    <s:if test="${requestScope.pb!=null}">
                        ...
                        <div class="pull-right"><!--右对齐--->
                            <ul class="pagination">
                                <li class="disabled"><a href="#">第<c:out
                                        value="${requestScope.pb.pageCode}"/>页/共<c:out value="${requestScope.pb.totaPage}"/>页</a>
                                </li>
                                <li>
                                    <a href="${pageContext.request.contextPath}/reader/borrowAction_${pb.url }pageCode=1">首页</a>
                                </li>
                                <li>
                                    <a href="${pageContext.request.contextPath}/reader/borrowAction_${pb.url }pageCode=${pb.pageCode-1 }">&laquo;</a>
                                </li><!-- 上一页 -->
                                    <%-- 循环显示页码列表 --%>
                                <c:forEach begin="${begin }" end="${end }" var="i">
                                    <c:choose>
                                        ...
                                        <c:otherwise>
                                            <li>
                                                <a href="${pageContext.request.contextPath}/reader/borrowAction_${pb.url }pageCode=${i}">${i}</a>
                                            </li>
                                        </c:otherwise>
                                    </c:choose>
                                </c:forEach>
                                ...
                                <c:if test="${pb.pageCode < pb.totaPage }">
                                    <li>
                                        <a href="${pageContext.request.contextPath}/reader/borrowAction_${pb.url }pageCode=${pb.pageCode+1}">&raquo;</a>
                                    </li>
                                </c:if>
                                    <%--否则显示尾页 --%>
                                <li>
                                    <a href="${pageContext.request.contextPath}/reader/borrowAction_${pb.url }pageCode=${pb.totaPage}">尾页</a>
                                </li>
                            </ul>
                        </div>
                    </s:if>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-7">
                            <input type="text" class="form-control" id="name" placeholder="请输入您的真实姓名"
                                   value='<c:out value="${sessionScope.reader.name}"/>'>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-7">
                            <input type="text" class="form-control" id="phone" placeholder="请输入您的联系号码"
                                   value='<c:out value="${sessionScope.reader.phone}"/>'>
                    ...
                    <div class="form-group">
                        ...
                        <div class="col-sm-7">
                            <input type="text" class="form-control" id="email" placeholder="请输入您的邮箱"
                                   value='<c:out value="${sessionScope.reader.email}"/>'>
