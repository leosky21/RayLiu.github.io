...
<script type="text/javascript">
		...
		$(function(){
			$('#dg').datagrid({   
			    url:Med+'/user/queryUser.action',
			    loadMsg:'正在查询用户信息......',
			    method: "post",
			    queryParams:{phone:''},//type参数，这里不需要传具体的type，因为我们要显示所有的
			    //width:300,
				...
				pageList:[5,10,15,20],
				idField:'phone',//指定id为标识字段，在删除，更新的时候有用，如果配置此字段，在翻页时，换页不会影响选中的项
				toolbar: [{
					iconCls: 'icon-add',
					text:'添加用户信息',
					handler: function(){
						parent.$("#win").window({
							...
							width:350,
							height:200,
							content:'<iframe src="'+Med+'/send/main/user/save" frameborder="0" width="100%" height="100%"/>'
						});
					}
				},'-',{
					iconCls: 'icon-edit',
					...
					handler: function(){
						...
						if(rows.length == 0) {
							...
							parent.$("#win").window({
								...
								width:350,
								height:250,
								content:'<iframe src="'+Med+'/send/main/user/update" frameborder="0" width="100%" height="100%"/>'
							});
							//2. 
						}
					}
					...
					handler: function(){
						...
						if(rows.length == 0) {
							...
							$.messager.confirm('删除的确认对话框', '您确定要删除此用户吗？', function(r){
								if (r){
								    ...
								    //2. 发送ajax请求
								    $.post(Med+"/user/deleteByUphone.action",{ids:ids},function(result){
								    	if(result == "true") {
			    ...
			    frozenColumns:[[
					...
					{field:'phone',title:'手机号码',width:100,  //字段hot
			    ...
			    //title：列标题，是显示给人看的
			    columns:[[    		           
			        {field:'nick',title:'用户登录名',width:80, 
			        ...
			        {field:'height',title:'身高(cm)',width:60,  //字段hot
						formatter: function(value,row,index){
							return "<span title=" +　value + ">" + value + "</span>";
						}
			        },
			        {field:'weight',title:'体重(kg)',width:60,  //字段hot
						formatter: function(value,row,index){
							return "<span title=" +　value + ">" + value + "</span>";
						}
			        },
			        {field:'drugallergyhistory',title:'药物过敏史',width:200,  //字段hot
						formatter: function(value,row,index){
							return "<span title=" +　value + ">" + value + "</span>";
						}
			        },
			        {field:'medicalhistory',title:'病史',width:200,  //字段hot
						formatter: function(value,row,index){
							return "<span title=" +　value + ">" + value + "</span>";
						}
			        }
